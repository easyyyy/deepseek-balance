...
if (apiKey) {
  addLine("--- 余额 API ---", new Color("#4f8cff"));
  try {
    const r1 = new Request("https://api.deepseek.com/user/balance");
    r1.headers = { "Authorization": "Bearer " + apiKey };
    const d1 = await r1.loadJSON();
    addLine("成功: balance=" + (d1.balance_infos?.[0]?.total_balance || "?"), new Color("#34d399"));
  } catch(e) {
    addLine("失败: " + e.message, new Color("#f87171"));
  }
} else {
...
if (userToken) {
  addLine("--- 消费 API ---", new Color("#a78bfa"));
  try {
    ...
    if (d2.code === 0) {
      addLine("成功! 消费: ¥" + d2.data?.biz_data?.monthly_costs?.[0]?.amount, new Color("#34d399"));
    } else {
      addLine("失败: " + d2.msg, new Color("#f87171"));
    }
  } catch(e) {
    addLine("失败: " + e.message, new Color("#f87171"));
  }
